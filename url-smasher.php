...
<?php
	...
	{
		print '<h3><strong>Settings for URL Smasher</strong></h3>';
//		print '<p><Enabling URL Smasher will automatically (without any effort on your part) convert all valid URLs to goo.gl short links. This will happen for pages, posts, and comments (depending on your settings below). Because you don\'t have to do anything special to smash a URL, this plugin is great for sites with lots of authors. URLs are smashed only during the post/page/comment save/update process; previously saved content does not smash URLs, unless you save that content again.</p>';
//		print "<p>All you need is a valid Google API Key; start <a href='https://developers.google.com/api-client-library/python/guide/aaa_apikeys' title='Get Your Own goo.gl API' target='_blank'>here</a>.</p>";
//		print "<p>If you don't use a valid Google API Key, URLs will not be smashed, they will be saved as entered. Be aware of Google's limit on the number of daily requests on your account.</p>";
//		print "<p>If there is a problem with the URL Smasher process, those URLs will not be changed. Once you solve the problem with your Google account (which is always the cause of URL Smashing not woorking), the next Save/Update of your content will smash the URLs.</p>";
//		print "<p>Any URL Smashed into a goo.gl URL will not be re-smashed, and will stay the same. URLs in comments are smashed only when the comment is initially saved or edited.</p>";
	...
	{
	   printf(
			'<input type="text" type="hidden" id="the_version" name="url_smasher_options[the_version]" value="1.00 (10-Aug-2015)" readonly="readonly" width="5" maxlength="5" />',
